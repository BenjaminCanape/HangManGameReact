...
class App extends Component {
   ...
   */
  componentDidMount() {
    document.onkeypress = event => this.handleLetterClick(event.key.toUpperCase());
  }
  ...
  handleLetterClick = value => {
    ...
    if (!won && error < 6 && ascii >= ASCII_A && ascii <= ASCII_Z) {
      ...
      if (!this.hasAlreadyBeenClicked(lettersClicked, value)) {
        ...
        this.setState({
          lettersClicked: newLettersClicked,
          attempt: newAttempt,
          error: newError,
          score: newScore,
          won: newWon,
      ...
      <div className="App">
        {won || error >= 6 ? (
          <div className="centralPanel">
            <span>
              {error >= 6 ? (
                <h1 className="finalScore">Tu as perdu</h1>
              ) : (
          ...
          <div>
            ...
            <span>
              Attention: Les citations à trouver sont en langue originale (pour l'instant Anglais et Français)<br />
              <br />
              Thème: Séries TV<br />
            ...
            <br />
            <span>
              <h1 className="word">{this.computeDisplay(word, lettersClicked)}</h1>
            </span>
            <br />
            <br />
            {letters.map((value, index) => (
              <Letter
                value={value}
                disabled={this.hasAlreadyBeenClicked}
                key={index}
